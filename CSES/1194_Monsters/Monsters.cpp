...
void solve() {
    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            dist[newi][newj] = min(dist[newi][newj], dist[f][s] + 1);
            q.push({newi, newj});
        }
    }
    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (newi < 0 || newi >= n || newj < 0 || newj >= m) continue;
            if (arr[newi][newj] == '#') continue;
            if (reached[newi][newj]) continue;
            if (dist[newi][newj] <= dist[f][s] + 1) continue;

            reached[newi][newj] = true;
            dist[newi][newj] = dist[f][s] + 1;
            trace[newi][newj] = dir[i];
...
outofloop:
    ...
    auto tr = end;
    while (tr != start) {
        s += trace[tr.first][tr.second];
        int i = 0;
        for (; i < 4; i++) if (dir[i] == trace[tr.first][tr.second]) break;
        tr = {tr.first + rev_i[i], tr.second + rev_j[i]};
    }
