...
void solve() {
    ...
    reverse(lastvis.begin(), lastvis.end());
    ...
    function<bool(int, int)> dfs3;
    dfs3 = [&](int curr, int dest) {
        if (curr == dest) return true;
        visited[curr] = true;

        for (auto s : adj[curr]) {
            if (visited[s]) continue;
            bool res = dfs3(s, dest);
            if (res) return true;
        }
        return false;
    };
